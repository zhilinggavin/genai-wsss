...
'''
    ...
    CSV file path: experiments/diffae_usage/encoded_shuffled/bs_10/shuffled_filenames_labels.csv
    Label 0: no_fibrosis
    Label 1: fibrosis
...
SAVE_DIR = f'experiments/tsne/tsne-20251011/tsne_bs_{BATCH_SIZE}_count{COUNT}'
...
WEIGHTED_TSNE: bool = True  # Whether to do weighted t-SNE visualization
...
for ms in MANIP_STRENGTH:    
    ...
    def tsne_and_plot(cond: np.ndarray, labels: np.ndarray, save_path: str, tsne_weight = None):
        ...
        tsne_all = tsne.fit_transform(cond)
        logging.info(f"TSNE transformed shape: {tsne_all.shape}")
        
        # Plot the embedded data
        fig, ax = plt.subplots(figsize=(6, 6), dpi=150)
        
        ax.scatter(tsne_all[mask1, 0], tsne_all[mask1, 1], color='red', label=f'Fibrosis({a1})', s=10)
        ax.scatter(tsne_all[mask0, 0], tsne_all[mask0, 1], color='green', label=f'No_Fibrosis({a2})', s=10)
        ax.scatter(tsne_all[maskm, 0], tsne_all[maskm, 1], color='blue', label=f'Manipulated({a3})', s=10)
    ...
    save_path = os.path.join(SAVE_DIR, 'imgs', f'manip{ms:.2f}_bs{BATCH_SIZE}_all600{weighted_str}{cls_check_str}.png')
