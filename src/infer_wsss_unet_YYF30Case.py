...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'name': name,
            'orig_img': torch.as_tensor(np.asarray(img).copy()).float().contiguous()
        }
        
def make_dataloader(case_dir: str, img_scale=0.5, ids: list = None):
    dataset = BasicDataset(case_dir, img_scale, ids=ids)
    assert len(dataset) > 0, f'No images found in {case_dir}'
...
def infer(model, data_loader, device, save_dir, csv_file_path=None):
    ...
    df['Case'] = [slice_name.rsplit('_', 1)[0] for slice_name in slcie_ID]
    df['ID'] = slcie_ID
    df['size'] = IMAGE_SIZE
    ...
    df_load = pd.read_csv(csv_file_path) if header_exists else None
    if header_exists and df['Case'].iloc[0] in df_load['Case'].values:
        return print(f'Case {df["Case"].iloc[0]} already exists in {csv_file_path}')
    else:
