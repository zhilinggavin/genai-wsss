...
# Generate sampled w,s and imgs from noise
def save_sampled(G, device, num_img=100, num_once=1,save_each_imgs=False):
    random_state = 5
    rnd = np.random.RandomState(random_state)

    W = np.zeros((num_img, 512), dtype='float32')
    S = np.zeros((num_img, 8704), dtype='float32')
    with torch.no_grad():
        for i in tqdm(range(int(num_img/num_once))):
            z =  rnd.randn(num_once, G.mapping.z_dim)
            z = torch.from_numpy(z).to(device)
            w = G.mapping(z, None) # [seed, layer, component]
            encoded_styles = G.synthesis.W2S(w)
            s = torch.concat(list(encoded_styles.values()), axis=-1)
            W[(i*num_once):((i+1)*num_once),:] = w[:,0,:].detach().cpu().numpy()
            S[(i*num_once):((i+1)*num_once),:] = s.detach().cpu().numpy()

        os.makedirs('npy/osic/sample', exist_ok=True)

        np.save(f'./npy/osic/sample/W{num_img}.npy',W)
        np.save(f'./npy/osic/sample/S{num_img}.npy',S)
        print(f'sample W and S are saved')
        
        if num_once==num_img:
            with open(f'./npy/osic/sample/encoded_styles{num_img}.pkl','wb') as f:
                pickle.dump(encoded_styles,f)
            print(f'encoded_styles{num_img} saved!')
        else:
            # W = np.load('./npy/osic/sample/W_100.npy')
            all_images=[]
            for i in tqdm(range(int(num_img/num_once))):
                images=[]
                w =  torch.from_numpy(W[(i*num_once):((i+1)*num_once),:]).unsqueeze(1).repeat(1,14,1).to(device)
                output = G.synthesis(w, noise_mode='const')
                # # Normalize to [-1, 1]
                # img = output; img = 2 * (img - img.min()) / (img.max() - img.min()) - 1
                img_out = (output.permute(0,2,3,1) * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()
                for j in range(num_once):
                    out = img_out[j]
                    if save_each_imgs:
                        os.makedirs('sampled_imgs/osic', exist_ok=True)
                        plt.imsave('sampled_imgs/osic/{:05d}.jpg'.format(i*num_once+j), out)
                        print(f'each sampled image is saved')
                    images.append(out)
                all_images.append(images)

            all_images=np.concatenate(all_images)
            np.save(f'npy/osic/sample/images_{num_img}.npy',all_images)
            print(f'all_images.shape = {all_images.shape}')

# grad_torch author method
# Compute the gradient map for each images along all S layers. Input:s vectors in each layer; Output:output image from G.
def compute_jacob_t(x, y):
    ...
    """
    jacobian = torch.zeros(y.numel(), x.numel())
    y = y.view(-1)  # Reshape y to a 1D tensor
    x.grad = None  # Clear x's gradient before the computation  
    
    for i in range(y.numel()):
        y[i].backward(retain_graph=True)  # Compute the gradient of y[i] with respect to x
        jacobian[i] = x.grad
        x.grad.zero_()  # Clear x's gradient for the next iteration

    jacob_t = jacobian.T.numpy()
    return jacob_t
...
def model_load(device, diff=True, cls=True):
    ...
    if diff:
        ...
        conf.name = 'test_osic256_cluster_mxl_round2'
        # state = torch.load(f'/media/NAS04/yyfang/prognostic_result/xai/counterfactual/Diffusion-Explainer/checkpoints/{conf.name}/last.ckpt', map_location='cpu')
        state = torch.load('/media/NAS06/gavinyue/disentanglement/scripts_segmentation/diffusion_checkpoints/last.ckpt', map_location='cpu')
        model_diff.load_state_dict(state['state_dict'], strict=False)
        model_diff.ema_model.eval()
        model_diff.ema_model.to(device)
