...
def process_anno():
    """Main function to process fibrosis images annotation."""
    work_dir = get_workspace_dir()
    save_dir = Path(work_dir) / 'data/OSIC/processed/fibrosis_anno'
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    for image_axial, labels, names in tqdm(islice(trainset_loader, max_count), total=max_count):
        ...
        # Process each image in the batch
        for img_id, name in enumerate(anno_names):
            orig_img_path = Path(trainset_loader.dataset.img_folder) / name
            ...
            try:
                orig_img = Image.open(orig_img_path)
                # Resize the GT annotation image using NEAREST interpolation.
                # Previously used LANCZOS, which caused non-unique values in the segmentation labels.
                # resized_img = trans_fn.resize(orig_img.convert("RGB"), IMAGE_SIZE, InterpolationMode.LANCZOS)
                resized_img = orig_img.resize((IMAGE_SIZE,IMAGE_SIZE), Image.NEAREST)             
                resized_img.save(new_img_path)
